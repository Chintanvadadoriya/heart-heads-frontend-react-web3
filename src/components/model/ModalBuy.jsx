...
import { buyNFT, getCurrencyInfoFromAddress } from "../../contract";
...
const ModalBuy = (props) => {
	...
	const { account, active, library, balance,error } = useActiveWeb3React();
	const { item, pairItem, setPairItem, showBuyNowModal, setShowBuyNowModal } =
		props;
	...
	if(balance){
		console.log("balance1612", balance);
	}else{
		console.log("balanceErrorHandle1612",error)
		console.error("balanceErrorHandle1612",error)
	}
	...
	async function buyNFTItem() {
		...
		if (
			(buyQuantity < 1 && item?.type === "multi") ||
			buyQuantity > pairItem.balance
		) {
		...
		const { data } = await axios.get(
			`${process.env.REACT_APP_API}/signature`,
			{
				params: {
					account: account,
					amount: buyQuantity,
		...
		if (data.status) {
			...
			buyNFT(
				...
				if (result) {
					...
					setPairItem(null);
					toast.success(
						"Buy Success! Data will be synced after some block confirmation..."
					);
					await sleep(2000);
					...
					<Element.ModalBody>
						...
						{item.type === "multi" && (
							<Element.Field>
								...
								>
									Enter quantity{" "}
									<span>({pairItem.balance} available)</span>
								</Element.Label>
								<Element.Input
									value={buyQuantity}
									type={"number"}
						...
						<Element.ModalRow>
							...
							<Element.ModalPrice
								className={`text_color_1_${theme}`}
							>
								{formatNum(balance)} {currencyInfo.symbol}
							</Element.ModalPrice>
						</Element.ModalRow>
						<Element.ModalRow>
							<Element.ModalLabel
							...
							<Element.ModalPrice
								className={`text_color_1_${theme}`}
							>
								{formatNum(pairItem.price * buyQuantity)}{" "}
								{currencyInfo.symbol}
							</Element.ModalPrice>
						</Element.ModalRow>
